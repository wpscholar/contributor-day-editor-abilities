...
test.describe( 'block type discovery', () => {
	test( 'editor/get-block-types lists registered blocks and supports search', async ( {
		...
		expect(
			all.value.blockTypes.some( ( type: { name: string } ) => type.name === 'core/paragraph' )
		).toBe( true );

		const filtered = await callTool( 'editor_get-block-types', { search: 'paragraph' } );
	...
	test( 'editor/get-block-types narrows to what a parent block accepts', async ( { callTool } ) => {
	...
	test( 'editor/get-block-type returns the full definition for one block', async ( {
		...
		const result = await callTool( 'editor_get-block-type', { name: 'core/paragraph' } );
		expect( result.isError ).toBe( false );
		expect( result.value.name ).toBe( 'core/paragraph' );
		expect( result.value.attributes ).toHaveProperty( 'content' );
	} );

	test( 'editor/get-block-type fails for an unregistered block name', async ( { callTool } ) => {
		const result = await callTool( 'editor_get-block-type', { name: 'not/a-real-block' } );
